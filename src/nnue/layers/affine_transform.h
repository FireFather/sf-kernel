...
namespace Nebula::Eval::Nnue::Layers{
  ...
  static void affineTransformNonSsse3(std::int32_t* output, const std::int8_t* weights, const std::int32_t* biases,
    const std::uint8_t* input){
# ifdef USE_SSE2
    constexpr IndexType NumChunks=ceilToMultiple<IndexType>(InputDimensions,16)/16;
    const __m128i Zeros=_mm_setzero_si128();
    const auto inputVector=reinterpret_cast<const __m128i*>(input);
# elif defined(USE_MMX)
    constexpr IndexType NumChunks=ceilToMultiple<IndexType>(InputDimensions,8)/8;
...
# else
      ...
      for (IndexType j=0; j<InputDimensions; ++j){ sum+=weights[offset+j]*input[j]; }
      output[i]=sum;
# endif
    }
# ifdef USE_MMX
...
# endif
  ...
  class AffineTransform<InDims, OutDims, std::enable_if_t<ceilToMultiple<IndexType>(InDims,maxSimdWidth)>=
      largeInputSize>>{
  public:
    ...
    static constexpr IndexType PaddedInputDimensions=
      ceilToMultiple<IndexType>(InputDimensions,maxSimdWidth);
    static constexpr IndexType PaddedOutputDimensions=
      ceilToMultiple<IndexType>(OutputDimensions,maxSimdWidth);
    ...
    static IndexType getWeightIndex(const IndexType i){
    ...
    bool readParameters(std::istream& stream){
      for (IndexType i=0; i<OutputDimensions; ++i)
        biases[i]=readLittleEndian<BiasType>(stream);
      for (IndexType i=0; i<OutputDimensions*PaddedInputDimensions; ++i)
        weights[getWeightIndex(i)]=readLittleEndian<WeightType>(stream);
      return !stream.fail();
    }
...
# undef vec_haddx4
  ...
  public:
...
#endif
    static constexpr std::uint32_t getHashValue(const std::uint32_t prevHash){
      std::uint32_t value=0xCC03DAE4u;
    ...
    static IndexType getWeightIndexScrambled(const IndexType i){
      return
    ...
    static IndexType getWeightIndex(const IndexType i){
#ifdef USE_SSSE3
      return getWeightIndexScrambled(i);
#else
      return i;
#endif
    }
    ...
    bool writeParameters(std::ostream& stream) const{
      for (IndexType i=0; i<OutputDimensions; ++i)
        writeLittleEndian<BiasType>(stream,biases[i]);
      for (IndexType i=0; i<OutputDimensions*PaddedInputDimensions; ++i)
        writeLittleEndian<WeightType>(stream,weights[getWeightIndex(i)]);
      return !stream.fail();
    }
...
#define vec_haddx4 Simd::m128_haddx4
      ...
      if constexpr (OutputDimensions%OutputSimdWidth==0){
        constexpr IndexType NumChunks=ceilToMultiple<IndexType>(InputDimensions,8)/4;
